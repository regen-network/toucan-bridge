...
contract ToucanRegenBridge is Ownable, Pausable {
    ...
    event TokenIssuer(address oldIssuer, address newIssuer);
    ...
    /**
     * @dev Sets the values for {tokenIssuer} and {toucanContractRegistry}.
     */
    constructor(
        ...
        if (tokenIssuer_ != address(0)) {
            emit TokenIssuer(address(0), tokenIssuer_);
        }
    }
    ...
    function setTokenIssuer(address newIssuer) external onlyOwner {
        ...
        emit TokenIssuer(oldIssuer, newIssuer);
    }
