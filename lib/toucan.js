...
async function prepareToucanEnv(admin, broker) {
	...
	for (const project of genesis) {
		const projectId = await deployProject(contracts.projects, project, admin.address, admin.address);
		...
		for (const vintage of project.Vintages) {
			...
			for (const batch of vintage.Batches) {
				await mintBatch(contracts.batches, batch, vintageId, admin, broker, tco2);
			}
		}
	}
...
async function deployProject(cProjects, project, adminAddress, beneficiaryAddress) {
...
async function mintBatch(cBatches, batch, vintageId, admin, broker, tco2) {
	if (batch.Owner != null) {
...
module.exports = { prepareToucanEnv, deployFixedContracts };
